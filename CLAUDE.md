...
- **Demo bots are server-simulated presence, not real clients.** `botTick` auto-spawns a fleet in the `demo` room (anchored to the first real user's fix; NYC fallback is free via the client) and moves them deterministically; bots are just extra `presence` rows, so the engine clusters them with **zero special-casing**. Three kinds (`BOT_*` constants): `roamer` (a group walks a shared loop **together** while clustered → a moving huddle that lays a heat **trail**), `wanderer` (spread out, orbit solo → movement only, no heat), `resident` (a group sits together at a fixed spot → a standing huddle = persistent hub). They mint synthetic identities (`new Identity(ctx.random.bigintInRange(...))`) since a reducer can't call `heartbeatLocation` (keyed on `ctx.sender`) for a bot. "Is a human around?" is keyed on presence **recency** (lastSeen within the stale window), not connection `status`. `botTick` also **config-refreshes** (despawn→respawn) when the live fleet no longer matches the spawn config, so a publish migrates bots without a wipe.
- **Heat is huddle-driven and trails behind movement.** The single heat writer `addHeat` is called **only** by the huddle engine — each member of an `active` huddle deposits weight at **their own position** every tick (`HEAT_PER_HUDDLE_TICK`), so a moving huddle paints a trail and a standing one builds a hub. A lone user/bot generates **no** heat. `heat_cell` weight accumulates (cap `HEAT_MAX`), **slow-fades** (`decayHuddles`, `×HEAT_DECAY_FACTOR≈0.97`/10s ≈ 4-min half-life), and is **never bulk-cleared** (it persists through bot teardown/republish — only fades). Cells are ~90 m (`HEAT_CELL_DEGREES`) for smooth trails. **Client (`useMapboxMap`):** a **steady** base heatmap (full field) + a **pulsing** `activity-heat-live` overlay of only recently-updated cells (`lastUpdatedAt` within `LIVE_WINDOW_MS`), so the pulse means "activity happening here now" (trail head + active hubs) — not decoration.
