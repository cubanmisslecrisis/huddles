...
- **Demo bots are server-simulated presence, not real clients.** `botTick` auto-spawns a fleet in the `demo` room (anchored to the first real user's fix; NYC fallback is free via the client) and moves them deterministically; bots are just extra `presence` rows + `bumpHeat` calls, so the engine clusters them with **zero special-casing**. They mint synthetic identities (`new Identity(ctx.random.bigintInRange(...))`) since a reducer can't call `heartbeatLocation` (keyed on `ctx.sender`) for a bot. "Is a human around?" is keyed on presence **recency** (lastSeen within the stale window), not connection `status`, so a brief disconnect doesn't despawn the fleet. All tuning lives in the `BOT_*` constants block.
