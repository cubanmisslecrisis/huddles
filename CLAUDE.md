...
- `spacetimedb/src/index.ts` is the full proximity + huddle engine: `user` / `room` / `presence` (lat/lng/hasFix) / `huddle` / `huddle_member` / `event` / `score` / `heat_cell` / `bot`, plus the scheduled `huddleTick` (1s — the **single** engine clock) / `expireStalePresence` (5s) / `decayHuddles` (10s) / `botTick` (~1.5s — demo bots). Clients stream location via `heartbeatLocation`; the server clusters by proximity (union-find with hysteresis) and runs candidate→active→cooling→ended.
- **Demo bots are server-simulated presence, not real clients.** `botTick` auto-spawns a fleet in the `demo` room (anchored to the first real user's fix; NYC fallback is free via the client); bots are just extra `presence` rows, so the engine clusters them with **zero special-casing**. Two kinds (`BOT_*` constants): `group` (members hold a fixed **formation** — a triangle for 3 — around a center that **wanders organically** across the map via a sum of incommensurate sines — a meandering, **non-circular** path, never a ring — so the huddle moves and its heat spreads/trails while staying member-shaped) and `wanderer` (spread out, orbit solo → movement only, no heat). They mint synthetic identities (`new Identity(ctx.random.bigintInRange(...))`) since a reducer can't call `heartbeatLocation` (keyed on `ctx.sender`) for a bot. "Is a human around?" is keyed on presence **recency** (lastSeen within the stale window), not connection `status`. `botTick` also **config-refreshes** (despawn→respawn) when the live fleet no longer matches the spawn config, so a publish migrates bots without a wipe.
- **Heat is huddle-driven and member-shaped.** The single heat writer `addHeat` is called **only** by the huddle engine — for an `active` huddle, each tick it deposits at the **centroid** (heavier → hot middle) and at **each member's position** (the corners), so the heat reads as the huddle's footprint: a rounded polygon (triangle for 3), hottest at the centroid — **not** a circle. Because groups wander, that footprint moves, so heat spreads/trails across the map (not a few static spots), but the motion is non-circular so it never paints a ring. A lone user/bot generates **no** heat. `heat_cell` weight accumulates (cap `HEAT_MAX`), **slow-fades** (`decayHuddles`, `×HEAT_DECAY_FACTOR≈0.97`/10s ≈ 4-min half-life), and is **never bulk-cleared** (persists through teardown/republish — only fades). Cells are ~45 m (`HEAT_CELL_DEGREES`) so a huddle's shape resolves. **Client (`useMapboxMap`):** a single **steady** heatmap layer — tight render radius so the shape shows (not a blur), and **no pulsing** (the heatmap never animates).
...
- **Client = mobile-only shell, Tailwind v4 + tokens.** Ported from the `suryanewa/huddle` skeleton's mobile shell (desktop shell dropped). Styling is **Tailwind v4** (`@tailwindcss/vite`) with the oklch brand tokens + base layer in `src/index.css`; brand colors via `var(--color-*)` / `lib/theme.ts`. `@/` path alias → `src/` (in `vite.config.ts` + `tsconfig.app.json`). UI primitives (`components/ui/{button,input,checkbox}`) are hand-rolled — do **not** add `@base-ui/react`/`shadcn`. **Claymorphism** is the visual language (synced from the skeleton's newer cut): the `shadow-clay` CSS utility in `index.css` + a `clay` variant on the hand-rolled `Button` (defaults `clay: true`, tinted per-variant via `--clay-color`; the springy squish is a child `<span>`) + shared soft-surface strings in `lib/ui-styles.ts` — all hand-rolled CSS over a native `<button>`, still no base-ui. **Demo avatar photos:** `components/Avatar.tsx` renders a deterministic portrait (`photoFor(key)` → pool in `public/avatars/`, same hashing as `colorFor`) clipped to a disc; the hashed color is the ring + the fallback when a photo 404s. Pass `photo` to override (the current user uses `ME_PHOTO`/`you.png`). Map markers carry the look: `FriendMarker` shows the photo, `HuddleMarker` a member-photo cluster (`MapAvatar.memberKeys`). App derives view-models (`lib/view.ts`) from live tables and passes them to dumb panels; UI structure is `components/{map,shell,panels,lens,flows,ui}`. Static "places" stubs live in `lib/places-data.ts` (no backend yet).
