...
- **Demo bots are server-simulated presence, not real clients.** `botTick` auto-spawns a fleet in the `demo` room (anchored to the first real user's fix; NYC fallback is free via the client) and moves them deterministically; bots are just extra `presence` rows, so the engine clusters them with **zero special-casing**. Three kinds (`BOT_*` constants): `huddler` (cycle-converge near you → live huddle loop), `wanderer` (spread out, orbit solo → movement only), `resident` (fixed groups that sit together elsewhere → standing huddles). They mint synthetic identities (`new Identity(ctx.random.bigintInRange(...))`) since a reducer can't call `heartbeatLocation` (keyed on `ctx.sender`) for a bot. "Is a human around?" is keyed on presence **recency** (lastSeen within the stale window), not connection `status`, so a brief disconnect doesn't despawn the fleet.
- **Heat is huddle-driven, not movement-driven.** The single heat writer `addHeat` is called **only** by the huddle engine — an `active` huddle deposits weight at its centroid each tick, scaled by member count (`HEAT_PER_HUDDLE_TICK`). A lone user/bot generates **no** heat. `heat_cell` weight accumulates (cap `HEAT_MAX`) and **slow-fades** (`decayHuddles`, `×HEAT_DECAY_FACTOR≈0.97`/10s ≈ 4-min half-life) so hangout spots persist for minutes. The client (`useMapboxMap`) maps weight `0..HEAT_MAX` to a real intensity gradient; the heatmap's gentle radius "breathing" (rAF `heatmapPulse`) is a separate render effect.
