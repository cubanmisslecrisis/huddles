...
export function BottomNavIsland({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-40 px-4">
      {/* FAB buttons — float above the nav island, anchored bottom-right */}
      <div
        className="pointer-events-auto absolute right-4 flex flex-col items-center gap-2.5"
        style={{ bottom: 'calc(env(safe-area-inset-bottom, 0px) + 16px + 68px + 12px)' }}
      >
        <button
          onClick={onSearch}
          aria-label="Search"
