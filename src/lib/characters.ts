// Club Penguin characters used as avatars. Each user gets a stable character
// derived from their identity hex. Each persona in Wrapped maps to one character.
...
// Context holding all identity hex keys currently in the room (sorted).
// When provided, characters are assigned by sort-position so nobody shares one.
export const CharacterKeysContext = createContext<string[]>([]);

// Returns the unique character for `key`. If the context has all room keys,
// assignment is by sort-index (guaranteed unique up to 8 users). Otherwise
// falls back to a hash so it still works without the context.
...
// Plain function for non-hook contexts (e.g. Wrapped persona).
...
// Deterministic persona from week seed (used in Wrapped).
export function personaFor(seed: number): Character {
  return CHARACTERS[((seed * 2654435761) >>> 0) % CHARACTERS.length];
}
